class GameController < ApplicationController
  ...
  def joinTheGame
    ...
    if @current_user.id == invitation.user_id && game.id == invitation.game_id
      ...
      stories.map do |story|
        answers[story.id] = if game['selected_story'] && game['selected_story']['id']
                              []
                            else
                              story.answers
                            end
  ...
  def searchGameYouHaveJoined
    ...
    if !!invitation
      ...
      answers = {}
      stories.map do |story|
        answers[story.id] = if game['selected_story'] && game['selected_story']['id'] == story.id
                              []
                            else
                              story.answers
                            end
